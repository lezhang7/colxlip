...
class ColClipLoss(nn.Module):
    ...
    def get_logits(self, image_features, text_features, token_image_features, token_text_features, logit_scale, logit_bias=None):
        if self.world_size > 1:
            all_image_features, all_text_features = gather_features(
                image_features,
                text_features,
            ...
            all_token_image_features, all_token_text_features = gather_features(
                token_image_features,
                token_text_features,
                local_loss=self.local_loss,
                gather_with_grad=self.gather_with_grad,
                rank=self.rank,
            ...
            if self.local_loss:
                ...
                logits_per_text_token = logit_scale * compute_colbert_similarity(all_token_image_features, all_token_text_features)
            ...
            logits_per_text_token = logit_scale * compute_colbert_similarity(token_image_features, token_text_features)
            logits_per_image_token = logits_per_text_token.T
        ...
        return {"logits_per_image": logits_per_image, "logits_per_text": logits_per_text, "logits_per_image_token": logits_per_image_token, "logits_per_text_token": logits_per_text_token}

    def forward(
            self,
            image_features=None,
            text_features=None,
            token_image_features=None,
            token_text_features=None,
            logit_scale=None,
            logit_bias=None,
            output_dict=False,
            **kwargs,
        ...
        device = image_features.device
        output = self.get_logits(image_features, text_features, token_image_features, token_text_features, logit_scale)
        labels = self.get_ground_truth(device, output["logits_per_image"].shape[0])
        global_contrastive_loss = (
            F.cross_entropy(output["logits_per_image"], labels) +
            F.cross_entropy(output["logits_per_text"], labels)
        ) / 2

        token_contrastive_loss = (
            F.cross_entropy(output["logits_per_image_token"], labels) +
            F.cross_entropy(output["logits_per_text_token"], labels)
        ) / 2
        total_loss = self.alpha * global_contrastive_loss + (1 - self.alpha) * token_contrastive_loss

        return {"global_contrastive_loss": global_contrastive_loss, "token_contrastive_loss": token_contrastive_loss, "total_loss": total_loss} if output_dict else total_loss
