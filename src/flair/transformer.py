...
class AttentionalPooler(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # print("query shape:", q.shape) # (256, 512) -> (128, 256, 512) after unsqueezed
        # print("x shape for key and value", x.shape) # (128, 197, 768)
        #out = self.attn(q.unsqueeze(0).expand(N, -1, -1), x, x, need_weights=False)[0]
        attn_outputs, attn_output_weights = self.attn(q.unsqueeze(0).expand(N, -1, -1), x, x, need_weights=False)
...
class VisionPostProcess(nn.Module):
    def __init__(
            self,
            context_dim: int,
            output_dim,
        ...
        self.proj = nn.Parameter(self.scale * torch.randn(context_dim, output_dim))
        if not self.skip_ln:
            self.ln_post = norm_layer(context_dim)
...
class TextPostProcess(nn.Module):
    def __init__(
            self,
            context_dim: int,
            output_dim,
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.width = width  #added, to indicate the dim before pooling
        self.skip_final_pooling = skip_final_pooling
        ...
        self.attn_pool_type = None
        self.proj_tokens = project_tokens
        ...
        if attentional_pool:
            if isinstance(attentional_pool, str):
                ...
                self.attn_pool = AttentionalPooler(
                    ...
                    width,
                    n_head=attn_pooler_heads,
                    n_queries=1,  #origin: attn_pooler_queries
                )
                self.attn_pool_contrastive = None
                pool_dim = output_dim
        else:
    ...
    def forward(self, x: torch.Tensor, return_all=False):
        ...
        if return_all:
            return x #directly returning the output after transformer, mainly used for segmentation

        if self.attn_pool is not None:  # for FLAIR, self.attn_pool is set to be 'None', because we are not using the implementation from open_clip
            ...
            elif self.attn_pool_type == 'base_attn_pool' or 'learnable_query':
                x_mean = x.mean(dim=1, keepdims=True)  #in learnable query, the input q does not matter
                pooled = self.attn_pool(x_mean, x, x).squeeze(1)
            else:
                # this is the original OpenCLIP CoCa setup, does not match paper
                x = self.attn_pool(x)
                # print("shape after attn_pool:", x.shape)
                x = self.ln_post(x)  # ln_post() originbally after self.attn_pool()
                pooled, tokens = self._global_pool(x)
                # print("shape of pooled", pooled.shape)
                pooled = pooled.squeeze(1)
                # print("shape of pooled after squeeze", pooled.shape)
        elif self.text_con:
            x = self.ln_post(x)
            if self.skip_final_pooling:
                pooled = None
                tokens = x
            else:
                pooled, tokens = self._global_pool(x)
            ...
            if self.proj is not None:
                ...
                # print("shape after projection:", pooled.shape)
